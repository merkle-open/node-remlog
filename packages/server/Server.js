...
class Server {
  constructor(config = defaultConfig) {
    ...
    logger.info(
      `Attaching transport ${getTransportNameFromId(this.transportId)} ...`
    );
  ...
  attachRouter() {
    this.instance.use(
      "/.resources",
      express.static(path.join(__dirname, "views/resources"))
    );
    ...
    this.instance.get("/logs/:id.json", (req, res, next) => {
      fs.readFile(GENERIC_TRANSPORT_LOGFILE, (err, data) => {
        ...
        if (!selected) {
          return next(
            new Error(`Logfile with ID ${req.params.id} was not found`)
          );
        }
